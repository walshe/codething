...
public class TrackerApp {
	...
	public void addActivity(final String userId, final Activity activity) {
		Optional.ofNullable(getUser(userId)).ifPresent(user -> user.addActivity(activity));
	}

	public int totalActivities(final String userId) {
		return Optional.ofNullable(getUser(userId)).map(User::totalActivities).orElse(0);
	}
	...
	public int totalDurationInMinutes(String userId) {
		return Optional.ofNullable(getUser(userId)).map(User::totalDurationInMinutes).orElse(0);
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			Arrays.asList(emmett, andy).forEach(user -> {
				...
				System.out.println(user.getUserId() + "'s Total Duration: "
						+ app.totalDurationInMinutes(user.getUserId()) + " minutes");
				System.out.println(user.getUserId() + "'s Total Calories Including BMR: "
						+ app.totalCaloriesBurnt(user.getUserId(), true) + " calories");
				System.out.println(user.getUserId() + "'s Total Calories Excluding BMR: "
						+ app.totalCaloriesBurnt(user.getUserId(), false) + " calories");
			});
		} catch (Exception e) {
